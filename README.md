...
> - **[`notes/topics/543-reconnect-ws-server-host-takeover.md`](notes/topics/543-reconnect-ws-server-host-takeover.md)** — WS-опкод 3 переписывает `server.host`/`server.port`/`server.useTls` в SharedPreferences без валидации (нет whitelist, нет regex, нет pinning), переживает logout+re-login (явная конструкция «сохранить → clear → восстановить» в `ri9.c()`), разрешает TLS downgrade на raw TCP. Scope: все 159 опкодов основного протокола. Адресный server-side MITM.
...
> **🔥🔥🔥 17 мая 2026 (вечер) — волна 3.** Реверс через rizin/r2 (декомпиляция .so, не только strings) + JNI-bridge audit. Три РЕАЛЬНО новые находки сверх волн 1-2:
>
> - **[`notes/topics/545-three-audio-interception-channels.md`](notes/topics/545-three-audio-interception-channels.md)** — три параллельных канала перехвата сырого аудио в звонках. Главное: `AudioRecordSampleHook` (`aeb`) — это **production Java-API подписки на raw-PCM микрофона**, любой код в процессе через `((CopyOnWriteArraySet) aeb.b).add(new dyk(0, listener))` получает каждый 10ms-буфер с микрофона **до** NS/AEC/AGC и Opus-кодирования, без проверки caller'а. Второй канал `nativeDeviceDataIsRecorded` через `AudioPlaybackCaptureConfiguration` захватывает аудио **других приложений** (USAGE_MEDIA + USAGE_GAME) — но **с явным UI switch «Транслировать звук»** в ScreenShareBottomSheet, это стандартная фича «share computer sound» как в Discord/Zoom, **не backdoor** (поправка к первой версии заметки). Третий — `nativeSubmitDumpRequest` с 6 точек pipeline (включая входящий поток собеседника). Кастомный WebRTC.
> - **[`notes/topics/546-libqrcode-anti-tamper-code-hiding.md`](notes/topics/546-libqrcode-anti-tamper-code-hiding.md)** — нативная библиотека `libqrcode.so` (138 KB) под видом QR-генератора содержит **anti-tamper для CallsSDK**: проверка APK-подписи через `PackageManager.SigningInfo`, генерация session-seed `initializeSessionSeed(Context, byte[], byte[]) → byte[]` для звонков, привязанная к integrity APK. Java-сторона: `CallsSdkInitializer.System.loadLibrary("qrcode")`. Имя файла — обман, security-критическая логика спрятана. Объясняет, зачем MAX нужен собственный `download.max.ru` (своя подпись пройдёт integrity-check).
> - **[`notes/topics/547-webview-ssl-bypass-deepfake-tflite-key.md`](notes/topics/547-webview-ssl-bypass-deepfake-tflite-key.md)** — серверный PmsKey `web_app:ssl_check` отключает SSL-валидацию для всех WebView мини-приложений и `VideoWebViewScreen` (НЕ DevMenu, удалённо включаемый сервером per-user); popup-WebView без security-настроек (`allowContentAccess=true` по дефолту); `VideoWebViewScreen` с `MIXED_CONTENT_ALWAYS_ALLOW(0)`; FAQ WebView пропускает `max://` deeplinks без whitelist; **Wav2Lip V1/V3/V4 в production** — full lip-sync deepfake engine с face detection и landmark caching; **hardcoded AES key** `6f 8c c4 b7 19 d4 0d 16 d1 fc b9 ba bb c4 7d 7e` для расшифровки всех серверных .tflite моделей (любой реверсер расшифровывает что внутри).
>
> Чего волна 3 НЕ нашла: ServiceManager hidden API через прямые Binder transactions (DPS использует только NetworkCapabilities reflection), сбор IMEI/IMSI/MAC, ptrace других процессов в `libtracernative`, PSTN-перехват через ConnectionService (PhoneAccount регистрируется как SELF_MANAGED), SQLCipher или другое шифрование локальных БД. См. `notes/wave3/`.

> **🔥🔥🔥🔥 17 мая 2026 (ночью) — волна 4.** Параллельные субагенты с rizin/r2 на ML pipeline + JS-bridge + tech.kwik QUIC + нативные библиотеки. Что подтвердили без новых находок: tech.kwik QUIC (33 файла, чистый порт upstream Kwik кроме TLS bypass из 537 — теперь 537 дополнен полным трейсом `qse → z68 → tpi.b`), нативные библиотеки `libgleff/libffmpg/libimagepipeline/libnative-filters/libjlottie` (все clean или patched-without-backdoor), call recording через RecordManager (имеет UI-индикатор; реальный covert-канал — `collect-debug-dump` 531). Новые находки 548-551:
>
> - **[`notes/topics/548-ml-pipeline-circular-md5-trust.md`](notes/topics/548-ml-pipeline-circular-md5-trust.md)** — server-pushed ML pipeline в звонках через RemoteSettings API: URL модели и ожидаемый MD5 checksum приходят из одного и того же серверного JSON-конфига (circular trust). Нет криптографической подписи моделей, нет whitelist доменов URL, нет certificate pinning, нет проверки типа engine в `.cfg`. 20+ engine типов в native Registry (`Pipeline`, `KWS`, `ASR`, `AudioClassifier`, `SpeakerRecognitionEngine`, `Wav2LipV1/V3/V4`, и др.) активируются строковым именем из `.cfg`. **В 26.16.0 KWS удалён из Java, но остался в native** — теоретически реактивируется через NS-канал. Расшифровка серверно-доставленных `.tflite` через hardcoded AES (см. 547).
> - **[`notes/topics/549-webapp-openmaxlink-deeplink-injection.md`](notes/topics/549-webapp-openmaxlink-deeplink-injection.md)** — JS-bridge метод `WebAppOpenMaxLink` принимает произвольный URL от мини-аппы без валидации (нет whitelist доменов, нет regex по схемам) и передаёт во внутренний deeplink-роутер `:link-intercept`. Любая мини-апа может программно навигировать пользователя по любому `max://`-deeplink или `https://max.ru/` внутреннему роуту, включая `:auth?externalCallback=1` (IDP-flow). Сравнение: Telegram `openTelegramLink` ограничен доменом `t.me`. User-click 3s обходится через `webapp-exc` (550).
> - **[`notes/topics/550-webapp-exc-user-click-bypass.md`](notes/topics/550-webapp-exc-user-click-bypass.md)** — PmsKey `webapp-exc` = серверный список `botId`, освобождённых от 3-секундного user-click requirement для 5 опасных JS-bridge методов (`WebAppOpenLink`, `WebAppOpenMaxLink`, `WebAppShare`, `WebAppMaxShare`, `WebAppDownloadFile`). Сервер per-bot выключает клиентскую защиту в runtime через NOTIF_CONFIG (134). Без UI-индикации, что бот в exempt-list. Превращает клиентскую защиту в server-side rubber-stamp.
> - **[`notes/topics/551-webapp-nfc-emulate-no-whitelist.md`](notes/topics/551-webapp-nfc-emulate-no-whitelist.md)** — `WebAppNfcEmulateNfcTag` доступен через **публичный** JS-bridge `WebViewHandler` (любая мини-апа), нет user-confirm, нет whitelist, нет user-click окна. Любая мини-апа может запустить эмуляцию NFC-тэга (`HostApduService`) с произвольными hex-данными. Системные ограничения Android HCE (foreground, AID-filter, payment category) снижают impact, но не устраняют возможность для категории `other`.
