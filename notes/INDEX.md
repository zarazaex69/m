...
> - **[[545-three-audio-interception-channels|545 — 3 канала перехвата аудио]]** — `AudioRecordSampleHook` (Java open-API подписки на raw PCM микрофона без проверки caller'а — это и есть основная аномалия) + `nativeDeviceDataIsRecorded` (захват аудио ДРУГИХ приложений через AudioPlaybackCaptureConfiguration — но **с явным UI switch «Транслировать звук»**, стандартная фича «share computer sound», не backdoor — поправка к первой версии заметки) + `nativeSubmitDumpRequest` (server-triggered).
> - **[[546-libqrcode-anti-tamper-code-hiding|546 — libqrcode.so code hiding]]** — anti-tamper / signature-check / session-seed для CallsSDK спрятан в библиотеке с обманным именем «qrcode». Намеренное code hiding security-критической логики.
> - **[[547-webview-ssl-bypass-deepfake-tflite-key|547 — WebView SSL bypass + Wav2Lip + TFLite key]]** — серверный PmsKey `web_app:ssl_check` отключает SSL-валидацию во всех WebView; popup-WebView без hardening; VideoWebViewScreen с MIXED_CONTENT_ALWAYS_ALLOW; FAQ WebView пропускает `max://` deeplinks; Wav2Lip V1/V3/V4 deepfake engine в production; hardcoded AES key для расшифровки всех серверных .tflite моделей.

> **🔴🔴🔴 Волна 4 (17 мая 2026 ночью).** Параллельные субагенты с rizin/r2 на ML pipeline + JS-bridge + tech.kwik QUIC + нативные библиотеки. Подтверждено: tech.kwik QUIC и нативные либы (libgleff/libffmpg/libimagepipeline/libjlottie) — **чистые, кроме TLS bypass из 537** (дополнен полным трейсом `qse → z68 → tpi.b`). Новые находки 548-551:
>
> - **[[548-ml-pipeline-circular-md5-trust|548 — ML pipeline circular MD5 trust]]** — server-pushed ML pipeline в звонках: URL модели и ожидаемый MD5 приходят из одного и того же серверного JSON-конфига. Нет криптографической подписи, нет whitelist URL, нет проверки типа engine в `.cfg`. 20+ engine типов в native Registry активируются из `.cfg`. В 26.16.0 KWS удалён из Java, **но остался в native** — теоретически реактивируется через NS-канал.
> - **[[549-webapp-openmaxlink-deeplink-injection|549 — WebAppOpenMaxLink deeplink injection]]** — JS-bridge метод принимает произвольный URL без валидации/whitelist и передаёт во внутренний deeplink-роутер `:link-intercept`. Любая мини-апа может программно навигировать пользователя по любому deeplink. Сравнение: Telegram `openTelegramLink` ограничен `t.me`.
> - **[[550-webapp-exc-user-click-bypass|550 — webapp-exc PmsKey]]** — серверный список botId, освобождённых от 3-секундного user-click requirement для 5 опасных JS-bridge методов. Сервер per-bot выключает клиентскую защиту в runtime через NOTIF_CONFIG.
> - **[[551-webapp-nfc-emulate-no-whitelist|551 — NFC HCE без whitelist]]** — `WebAppNfcEmulateNfcTag` доступен ВСЕМ мини-аппам через публичный bridge. Нет user-confirm, нет whitelist, нет user-click окна. Любая мини-апа может эмулировать NFC-тэг с произвольными данными.
...
- [[topics/533-doh-c2-endpoint-redirect]] — **DoH C2 REDIRECT**: TXT-запись `api._endpoint.ok.ru.` через `dns.google.com/resolve` → новый API-endpoint, classic malware C2 pattern, обход блокировок. Гейтится PmsKey `net-client-dns-enabled` #critical #c2 #dns #doh #server-control
- [[topics/534-server-webview-js-injection]] — **JS INJECTION RCE**: сервер выполняет произвольный JS через `evaluateJavascript()` в любой мини-аппе. Привилегированный JS-bridge (PrivateWebViewHandler) даёт доступ к MSISDN, NFC HCE, биометрии, файлам, secure storage. Whitelist ботов — серверный (`availableBotForPrBridge`) #critical #rce #webview #js-bridge
- [[topics/535-logcat-capture-upload]] — **LOGCAT EXFILTRATION**: `g85.java` запускает `logcat -b all -v long -f <path>` (все буферы, до 32 МБ ротирующих файлов) → SampleUploadWorker → `sdk-api.apptracer.ru`. Активация серверным PmsKey `log-full`/`log-sensitive`/`user-debug-report`. Логи содержат данные ВСЕХ приложений на pre-Android 11 #critical #logcat #exfiltration #apptracer
- [[topics/536-widevine-hardware-fingerprint]] — **HARDWARE FINGERPRINT**: `kc5.java` через MediaDrm Widevine читает `deviceUniqueId` (зашит в TEE, несбрасываемый) → SHA-256 → 16 hex. Все строки обфусцированы XOR. Кросс-приложение fingerprint VK/OK/MAX. Используется в DPS #critical #fingerprint #widevine #hardware-id
- [[topics/537-quic-tls-bypass-mitm]] — **QUIC TLS BYPASS**: `qse.java` — пустой X509TrustManager (`checkServerTrusted(){}` → null), используется в `z68.java` для ВСЕХ QUIC-соединений (WebTransport). Приложение само пишет SECURITY WARNING в stdout, но конфигурация активна в production. Без PmsKey-гейтинга #critical #tls #quic #mitm #webtransport
- [[topics/538-invisible-messages-server-manipulation]] — **MESSAGE MANIPULATION**: NOTIF_MESSAGE+invisible=true → инъекция невидимых сообщений; NOTIF_MSG_DELETE → удаление без следа; перезапись по существующему msgId → подмена содержимого; CHAT_HIDE → скрытие чата; UPDATE_FIRE_TIME → ретроактивный TTL #critical #messages #server-control
- [[topics/539-fcm-c2-silent-location-tracking]] — **FCM C2 → SILENT GPS**: 7 типов push-команд, `LocationRequest` будит устройство (WakeLock в Doze через PmsKey `wakelock-on-push`) → WS reconnect → GPS fix → отправка координат. Без UI. `TamtamSpam` — произвольный URI без whitelist схем #critical #fcm #c2 #location #silent
- [[topics/540-webrtc-encryption-disable-stun-marking-vpn-bypass]] — **СОРМ-READY WebRTC**: 1) `getDisableEncryption` + `UnencryptedRtpTransport` (отключение SRTP), 2) `WebRTC-OK-StunCustomAttr` + `WebRTC-OK-TurnChannelDataMark` (серверные маркеры в STUN/TURN для DPI-идентификации), 3) `VpnPreference.NEVER_USE_VPN` (принудительный обход VPN), 4) `p2p_forbidden` (весь медиа через relay) #critical #webrtc #encryption #sorm #vpn
- [[topics/541-miniapp-disable-flag-secure]] — **FLAG_SECURE BYPASS**: JS-bridge команда `setup_screen_capture_behavior` снимает FLAG_SECURE со всего окна → screenshots/screen recording/MediaProjection/Recents-превью становятся доступны для всего MAX, включая чаты #critical #flag-secure #screen-capture #js-bridge
...
- [[topics/545-three-audio-interception-channels]] — **3 ПАРАЛЛЕЛЬНЫХ КАНАЛА AUDIO в звонках**: (1) **🔴 главная аномалия** — `AudioRecordSampleHook` (`aeb`) — Java production API подписки на raw PCM микрофона: `CopyOnWriteArraySet<dyk>` подписчиков, любой код может зарегистрировать listener через `add(new dyk(0, listener))` и получать каждый 10ms-буфер ДО передачи в native WebRTC, без проверки caller'а; (2) `nativeDeviceDataIsRecorded` — захват аудио ДРУГИХ приложений через `AudioPlaybackCaptureConfiguration` (Android 10+), **с явным UI switch «Транслировать звук»** (`call_share_sound_switch`) в ScreenShareBottomSheet — стандартная фича «share computer sound» как в Discord/Zoom, **не backdoor** (поправка к первой версии заметки); (3) `nativeSubmitDumpRequest` (см. 531) дополнен полным enum DumpSource с 6 точек pipeline. Кастомные WebRTC field trials `WebRTC-EncoderDataDumpDirectory` контролируют видео-дамп через серверный fieldTrials. Без изменений в 26.16.0 #critical #audio-interception #raw-pcm #microphone #parallel-sink #cross-app-audio #custom-webrtc
- [[topics/546-libqrcode-anti-tamper-code-hiding]] — **CODE HIDING**: `libqrcode.so` (138 KB, JNI namespace `one.me.sdk.uikit.qr.QrCodeGenerator`) — НЕ ТОЛЬКО QR-генератор. Внутри JNI_OnLoad динамически регистрируются native-методы для `one.me.callssdk.CallsSdkInitializer`: `IntegrityProtectionInit(Context)` (проверка APK-подписи через PackageManager.SigningInfo), `sign_check`, `initializeSessionSeed(Context, byte[], byte[]) → byte[]` (криптографический seed для сессии звонков, привязанный к integrity APK). Имя библиотеки — обман: security-critical логика спрятана под видом UI-компонента. Объясняет, почему MAX распространяется через собственный download.max.ru (своя подпись пройдёт integrity-check). Без сетевых вызовов в native (всё на Java) #high #anti-tamper #code-hiding #integrity #calls-sdk #signature-check #deception
- [[topics/547-webview-ssl-bypass-deepfake-tflite-key]] — **GROUP**: (1) PmsKey `web_app:ssl_check` — серверный SSL bypass для всех WebView через `xyc.onReceivedSslError → sslErrorHandler.proceed()`; НЕ DevMenu, удалённо включаемый PmsKey; адресный server-controlled MITM на WebView-трафик. (2) `syc.onCreateWindow` popup без security hardening — default `allowContentAccess=true`. (3) `VideoWebViewScreen` с `MIXED_CONTENT_ALWAYS_ALLOW(0)` + `allowContentAccess(true)`. (4) FAQ WebView пропускает `max://` deeplinks — FAQ-страница может триггерить произвольные deeplinks. (5) **Wav2Lip V1/V3/V4** — full lip-sync deepfake engine в production-сборке `libEnhancementLibShared.so`: face detection (TFLite Detection PostProcess) + landmark caching, без UI-toggle отключения. (6) **Hardcoded AES key** для encrypt/decrypt всех серверных .tflite моделей: `6f 8c c4 b7 19 d4 0d 16 d1 fc b9 ba bb c4 7d 7e` в .rodata @ 0x83513 — любой реверсер расшифровывает все скачанные модели. Без изменений в 26.16.0 #critical #webview #ssl-bypass #pmskey #server-control #mixed-content #deepfake #wav2lip #hardcoded-key #tflite

### 🔴🔴🔴 Волна 4 (17 мая 2026 ночью) — rizin/r2 + JS-bridge audit

- [[topics/548-ml-pipeline-circular-md5-trust]] — **ML pipeline circular MD5 trust**: server-pushed ML pipeline в звонках (KWS/NS) — URL модели и ожидаемый MD5 приходят из одного и того же серверного JSON-конфига `{url, cs, use}` через RemoteSettings API. Нет криптографической подписи моделей, нет whitelist URL, нет certificate pinning, нет проверки типа engine в `.cfg`. 20+ engine типов в native Registry (`Pipeline`, `KWS`, `ASR`, `AudioClassifier`, `DiarizationEngine`, `SpeakerRecognitionEngine`, `SpeakerRecognitionVerifier`, `Wav2LipV1/V3/V4`, `Profanity`, `ForcedAligner`, `Animoji`, `SuperResolution`, и др.) активируются строковым именем из `.cfg`. Java-слой не валидирует тип engine. В 26.16.0 KWS удалён из Java (KwsFeatureDelegate, MLFeatureType.WS), **но `vk::enh::KWS` остался в native** — теоретически реактивируется через NS-канал (NS-конфиг приносит `.cfg` с KWS engine). Расшифровка через hardcoded AES (см. 547) #high #ml-pipeline #server-control #circular-trust #no-signature #model-download #tflite
- [[topics/549-webapp-openmaxlink-deeplink-injection]] — **WebAppOpenMaxLink deeplink injection**: JS-bridge метод (`bek` → `udk` → `wik` → `uhk`) принимает произвольный URL без валидации (нет whitelist доменов, нет regex по схемам) и передаёт во внутренний deeplink-роутер `:link-intercept`. Любая мини-апа в `WebAppRootScreen` может программно навигировать пользователя по любому max://-deeplink или https://max.ru/ внутреннему роуту, включая `:auth?externalCallback=1` (IDP-flow). Сравнение: Telegram `openTelegramLink` ограничен `t.me`. User-click 3s обходится через `webapp-exc` (см. 550) #high #webview #js-bridge #deeplink-injection #no-whitelist
- [[topics/550-webapp-exc-user-click-bypass]] — **PmsKey `webapp-exc`**: серверный список botId, освобождённых от 3-секундного user-click requirement для 5 опасных JS-bridge методов (`WebAppOpenLink`, `WebAppOpenMaxLink`, `WebAppShare`, `WebAppMaxShare`, `WebAppDownloadFile`). Сервер per-bot выключает клиентскую защиту в runtime через NOTIF_CONFIG (134). Без UI-индикации что бот в exempt-list. Превращает клиентскую защиту в server-side rubber-stamp #medium #pmskey #server-control #user-click-bypass #webapp
- [[topics/551-webapp-nfc-emulate-no-whitelist]] — **WebAppNfcEmulateNfcTag без whitelist**: метод доступен через ПУБЛИЧНЫЙ bridge `WebViewHandler` (любая мини-апа), нет user-confirm, нет whitelist, нет user-click окна. Любая мини-апа может через `rfk.java` запустить эмуляцию NFC-тэга (`one.me.webapp.util.WebAppNfcService` HostApduService) с произвольными hex-данными. Системные ограничения Android HCE (foreground, AID-filter, payment category) снижают impact, но не устраняют возможность для категории `other`. В сочетании с 534 (server JS injection) — программное клонирование тэгов из любой мини-аппы #medium #webview #js-bridge #nfc-hce #no-whitelist
