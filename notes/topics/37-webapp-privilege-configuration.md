...
1. **`webapp-exc` — серверный allowlist привилегированных мини-апок.** Сервер пушит список chat-id, и каждой такой мини-апе клиент даёт расширенные права (например, без OS-prompt передачу MSISDN через `verify_mobile_id`, NFC HCE-атаку payload, доступ к контактам и т. п.). По умолчанию список пустой (`new long[0]`), но сервер может его наполнить в любой момент.
