...
## Дополнение: ConversationParams кодирование

`ConversationParams.decode(String)` — параметры звонка передаются в формате `<size>:<base64(LZ4(JSON))>`. Это означает, что параметры звонка (endpoint, STUN/TURN, ISP данные) сжимаются LZ4 и кодируются Base64. Это компактный формат для передачи через URL или push-уведомление.
